...
A dedicated module centralizes browser cache rules according to Jahia content URLs. The client caching strategy is preset by a filter according to matching rule. Some custom rules can be included for specific module's content URLs customize behavior for some specific module's URLs. Using filter's 'strict' mode ensures that other code cannot update header's values.
