...
echo " JAHIA_PROPERTIES: ${JAHIA_PROPERTIES}"
...
echo "$(date +'%d %B %Y - %k:%M') [CONTAINERS] == Start the other containers =="
...
if [[ "${JAHIA_CLUSTER_ENABLED}" == "true" ]]; then
    ...
    echo " JAHIA_PROCESSING_HOST: ${JAHIA_PROCESSING_HOST}"
    echo " JAHIA_PROCESSING_PORT: ${JAHIA_PROCESSING_PORT}"
